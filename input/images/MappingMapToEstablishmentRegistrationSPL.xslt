		...
		<xsl:if test="fn:exists(($var1_map_of_type)[.])">
			...
			<EstablishmentRegistration.EstablishmentEntity xmlns="urn:hl7-org:v3">
				<assignedOrganization>
					...
					<contactParty>
						<addr>
							<xsl:for-each select="$var17_contact/ns0:address/ns0:country">
								<country>
									<xsl:for-each select="@value">
										<xsl:sequence select="fn:string(.)"/>
									</xsl:for-each>
							...
							<xsl:for-each select="$var17_contact/ns0:address/ns0:state">
								<state>
									<xsl:for-each select="@value">
										<xsl:sequence select="fn:string(.)"/>
									</xsl:for-each>
							...
							<xsl:for-each select="$var17_contact/ns0:address/ns0:city">
								<city>
									<xsl:for-each select="@value">
										<xsl:sequence select="fn:string(.)"/>
									</xsl:for-each>
							...
							<xsl:for-each select="$var17_contact/ns0:address/ns0:postalCode">
								<postalCode>
									<xsl:for-each select="@value">
										<xsl:sequence select="fn:string(.)"/>
									</xsl:for-each>
							...
							<xsl:for-each select="$var17_contact/ns0:address/ns0:line">
								<streetAddressLine>
									<xsl:for-each select="@value">
										<xsl:sequence select="fn:string(.)"/>
									</xsl:for-each>
						...
						<xsl:for-each select="$var17_contact/ns0:telecom">
						...
						<contactPerson>
							<xsl:for-each select="$var17_contact/ns0:name">
								<name>
					...
					<xsl:for-each select="$var16_contained/ns0:Organization">
						...
						<assignedEntity>
							<assignedOrganization>
								<id>
									<xsl:for-each select="$var13_identifier/ns0:system/@value">
										<xsl:variable name="var6_cur_as_string" as="xs:string" select="xs:string(xs:anyURI(fn:string(.)))"/>
										<xsl:attribute name="root" namespace="">
											<xsl:choose>
												<xsl:when test="fn:starts-with($var6_cur_as_string, 'urn:oid:')">
													<xsl:sequence select="fn:substring-after($var6_cur_as_string, 'urn:oid:')"/>
												</xsl:when>
												<xsl:otherwise>
									...
									<xsl:for-each select="$var13_identifier/ns0:value/@value">
										<xsl:attribute name="extension" namespace="" select="fn:string(.)"/>
									</xsl:for-each>
								</id>
				...
				<xsl:for-each select="$var16_contained/ns0:HealthcareService">
					<performance>
						<actDefinition>
							<xsl:for-each select="ns0:type">
								<xsl:variable name="var14_coding" as="node()*" select="ns0:coding"/>
								<code>
									<xsl:for-each select="$var14_coding/ns0:code/@value">
										<xsl:attribute name="code" namespace="" select="fn:string(.)"/>
									</xsl:for-each>
									<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.3.26.1.1'"/>
									<xsl:for-each select="$var14_coding/ns0:display/@value">
										<xsl:attribute name="displayName" namespace="" select="fn:string(.)"/>
									</xsl:for-each>
								</code>
							</xsl:for-each>
							<xsl:for-each select="ns0:serviceProvisionCode">
								<xsl:variable name="var15_coding" as="node()*" select="ns0:coding"/>
								<subjectOf>
									<approval>
										<code>
											<xsl:for-each select="$var15_coding/ns0:code/@value">
												<xsl:attribute name="code" namespace="" select="fn:string(.)"/>
											</xsl:for-each>
											<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.3.26.1.1'"/>
											<xsl:for-each select="$var15_coding/ns0:display/@value">
												<xsl:attribute name="displayName" namespace="" select="fn:string(.)"/>
											</xsl:for-each>
										</code>
									</approval>
