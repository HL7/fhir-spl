			...
			<GDUFAFacilityIdentification.GDUFAFacilityEntity xmlns="urn:hl7-org:v3">
				<assignedOrganization>
					...
					<xsl:variable name="var4_resultof_filter" as="node()*">
						<xsl:for-each select="$var14_contact">
					...
					<performance>
						<actDefinition>
							...
							<xsl:for-each select="ns0:serviceProvisionCode">
								<xsl:variable name="var13_coding" as="node()*" select="ns0:coding"/>
								<subjectOf>
									<approval>
										<code>
											<xsl:for-each select="$var13_coding/ns0:code/@value">
												<xsl:attribute name="code" namespace="" select="fn:string(.)"/>
											</xsl:for-each>
											<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.3.26.1.1'"/>
											<xsl:for-each select="$var13_coding/ns0:display/@value">
												<xsl:attribute name="displayName" namespace="" select="fn:string(.)"/>
											</xsl:for-each>
										</code>
									</approval>
